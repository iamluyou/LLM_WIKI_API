...
async def run_ingest(source_ids: list[str], task: Task) -> TaskResult:
    ...
    for source_id in source_ids:
        task.progress = f"Processing {source_id}"
        ...
        if settings.ingest_cache_enabled and ingest_cache.check(raw_filename, content):
            ...
            logger.info(f"Cache hit for {raw_filename}, skipping")
            continue
...
async def _ingest_one(
    ...
    # Step 1: 分析
    task.progress = f"Analyzing {filename}"
    ...
    analysis_result, usage1 = llm_client.chat(analysis_messages)
    total_tokens["input"] += usage1.get("input", 0)
    total_tokens["output"] += usage1.get("output", 0)

    # Step 2: 生成
    task.progress = f"Generating wiki pages for {filename}"
    ...
    generation_result, usage2 = llm_client.chat(generation_messages, max_tokens=32000)
    ...
    for block in parse_result.blocks:
        ...
        # 语言守卫
        if not content_matches_target_language(block.content, settings.output_language):
            logger.warning(f"Language mismatch for {path}, skipping")
            continue

        # 写入策略
        if rel_path == "log.md":
            # 追加
            wiki_manager.append_to_file("log.md", block.content + "\n")
            pages_updated.append("log.md")
        elif rel_path in ("index.md", "overview.md"):
            # 覆盖
            wiki_manager.write_wiki_page(rel_path, block.content)
            pages_updated.append(rel_path)
        else:
            # 合并
            existing = wiki_manager.read_wiki_page(rel_path)

            def merger_fn(existing_content, new_content, source_name):
            ...
            merged, was_llm = merge_page_content(
